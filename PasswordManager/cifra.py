    ...
    if os.path.exists("passwords.dat"):
        with open("passwords.dat", "rb") as password_file:
            ...
            for encrypted_service, encrypted_username, encrypted_password in encrypted_data:
                ...
                password.append((service, username, password))
    return passwords

def add_password(passwords, key):
    service = input("Inserisci il nome di un servizio: ")
...
def view_password(passwords, key):
    service_name = input("Inserisci il nome del servizio di cui vuoi visualizzare la password: ")
    found = False
    for service, username, password in passwords:
...
def main():
    ...
    if choice == "SI":
        print("Seleziona il file secret.key")
        key = load_key()
    elif choice == "NO":
        print("chiave generata (secret.key). NASCONDILA e CONSERVALA, serve a recuperare le tue passwords")
        generate_key()
        print("Ora seleziona il file secret.key")
        key = load_key()
    else:
        return    
